---
layout: post
title: "IPv6 via Route64 Tunnelbroker on Ubuntu Server"
lang: "en"
author: "pljones"
heading: "Setting up IPv6 connectivity using Route64's Wireguard tunnelbroker"
---

> Copyright (C) 2026  Peter L Jones
> 
> This document is licesed under the Creative Commons Attribution-ShareAlike 4.0 International License.
...
The initial set up here was guided by Claude AI.  Testing was done with the help of @ann0see and @softins.
@dtinth has "field tested" the guide to set up a similar Route64 tunnel for the Asia Directory.

<!--more-->

If you need help with anything, first read the [Troubleshooting](#troubleshooting) section.
...
| `2001:db8:1400:af02::/64` … `77ff::/64` | Available for future use (Docker, DMZ, etc.) |
...
> [!NOTE]
> If Route64 does not have a hub near you, you should find an alternative tunnelbroker supplier.
> Adding the tunnel introduces latency, so choose a provider with a hub close to you for best performance.
...
and no external IPv6 connectivity (otherwise you would not need the tunnel).
...
- **Firewall** — ensure the router passes UDP traffic to the Wireguard port
...
The `PreferredSource` setting is critical — without it, reply packets from services on the
server use the tunnel transport address (`2001:db8:f06:af::2`) rather than your public
address (`2001:db8:1400:af00::1`). Remote clients will discard replies that come from an
...
The server will autoconfigure an EUI-64 address for itself, triggered by the RA
broadcast it sends to the LAN.
This causes source address selection problems
(and hence services reply from the wrong address).
To suppress the autoconfiguration, use this `systemd-networkd` drop-in:
...
Adjust the `en0` as needed for your LAN interface name:
```bash
...
It is probably worth working out what IPv4 ports you have open and start with the same set for IPv6.
You may also want to add counters.  These can be useful for debugging and monitoring.
This example includes Jamulus as a template for UDP services and a number of TCP services.
It also includes Apache/Nginx ports 80/443 for web servers, SSH, Samba, and a range of ports for other services.
...
(Check the ruleset with `sudo nft -c list ruleset` before applying, to catch syntax errors and
spot severe breakage.)
...
It should show your IPv6 address as `2001:db8:1400:af01:...` and score 10/10.
...
### Useful sites
- Route64 support: [https://route64.org/support](https://route64.org/support)
- HE.net looking glass: [https://lg.he.net/](https://lg.he.net)
- IPv6 test: [https://test-ipv6.com](https://test-ipv6.com)
