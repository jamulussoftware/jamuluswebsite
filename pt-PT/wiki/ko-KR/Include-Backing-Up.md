<!-- 참고: 이것은 클라이언트와 서버, 그리고 모든 운영 체제에 적용되어야 합니다 -->
...
Windows에서 사용자의 설정 파일을 찾으려면 검색창에 `%APPDATA%`를 입력하고 `Jamulus`라는 폴더를 찾습니다. 이
폴더에는 하나 이상의 `.ini` 파일이 있습니다. 이제 Jamulus 설정을 백업하십시오.
