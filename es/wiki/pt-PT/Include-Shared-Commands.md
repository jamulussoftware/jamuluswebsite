...
[comment]: # (Este é um ficheiro incluído para utilizar em múltiplos documentos)

- `-h` ou `--help` Exibe o texto da ajuda
...
- `-n` ou `--nogui` Desativa a GUI (para utilizar no modo simples)
...
- `-Q` or `--qos` Sets the quality of service DS Field byte. Default is 128
  ...
  page](QOS-Windows)
- `-t` or `--notranslation` Desativar as traduções do idioma da IU
