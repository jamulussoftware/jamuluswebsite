...
You can run as a Registered Server long enough for your band to connect, then go "private" (Unregistered) by restarting your server without the `--directoryserver` option (or by setting the Directory to "none" in the Server GUI). Your band mates will still be connected to the Server until they disconnect. (Thanks to [David Savinkoff](https://github.com/DavidSavinkoff) for this tip!)
