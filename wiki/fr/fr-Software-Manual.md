...
Vous pouvez regrouper les canaux audio en utilisant le bouton « groupe ». Si vous déplacez le potentiomètre d'un membre du groupe, les autres potentiomètres de ce groupe seront déplacés dans la même proportion.
...
Le pilote ASIO (« Périphérique ») peut être sélectionné à l'aide de Jamulus sous le système d'exploitation Windows. Si le pilote ASIO sélectionné n'est pas valide, un message d'erreur s'affiche et le pilote valide précédent est sélectionné. Sous le système d'exploitation MacOS, les périphériques d'entrée et de sortie peut être sélectionné.
...
Vous pouvez définir manuellement la taille du tampon de gigue pour le client local et le serveur distant. Pour la mémoire tampon de gigue locale, les pertes de flux audio sont indiquées par le voyant situé sous les curseurs de taille de la mémoire tampon de gigue.
...
Ce paramètre définit le niveau du canal d'un client nouvellement connecté, en pourcentage. Si un nouveau musicien se connecte au serveur, il obtiendra le niveau initial spécifié si aucun autre niveau de canal provenant d'une connexion précédente de ce même client n'a déjà été enregistré.
...
Le temps de réponse (ping) est le temps nécessaire au flux audio pour faire l'aller-retour entre le client et le serveur.
